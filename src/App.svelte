...
import { ObstacleAvoidanceSim } from "./lib/Simulation.js";
import Telemetry from "./lib/Telemetry.svelte";

let simulation = new ObstacleAvoidanceSim();

let params = $state({
	maxSpeed: 120,
	maxForce: 200,
	boundingRadius: 12,
	detectionBoxLength: 120,
	brakingWeight: 1.0,
	lateralMultiplier: 2.0,
	weightAvoidance: 3.0,
	weightSeek: 1.5,
	weightWander: 1.0,
	obstacleRadius: 30,
...
function handleClearObstacles() { simulation.clearObstacles(); }
function handleClearTarget() { simulation.clearTarget(); }

function handleKeydown(e) {
	if (e.key === "t") showTrail = !showTrail;
	if (e.key === "b") showDetectionBox = !showDetectionBox;
	if (e.key === "l") showLocalAxes = !showLocalAxes;
	if (e.key === "f") showForceVectors = !showForceVectors;
	if (e.key === "r") handleReset();
	if (e.key === "o") interactionMode = interactionMode === "obstacle" ? "select" : "obstacle";
	if (e.key === "g") interactionMode = interactionMode === "target" ? "select" : "target";
...
<main class="app-layout">
  ...
  <aside class="sidebar-left">
    ...
    <div class="app-footer">
      Made with &#10084;&#65039; for Swinburne &mdash; COS30002 Artificial Intelligence for Games
    </div>
  </aside>
  {/if}
  ...
  <section class="canvas-panel" bind:this={containerRef}>
    ...
    <Canvas
      ...
      bind:showTrail
      bind:showLocalAxes
      bind:showDetectionBox
      bind:showForceVectors
      bind:interactionMode
    />

    <div class="mode-badge" class:badge-obstacle={interactionMode === 'obstacle'} class:badge-target={interactionMode === 'target'}>
      {#if interactionMode === 'obstacle'}
        Obstacle mode &mdash; click to place / remove &nbsp; [Esc] to exit
      {:else if interactionMode === 'target'}
        Target mode &mdash; click to place / click again to remove &nbsp; [Esc] to exit
      {:else}
        [T] Trail &nbsp;&middot;&nbsp; [B] Box &nbsp;&middot;&nbsp; [L] Axes &nbsp;&middot;&nbsp; [F] Forces &nbsp;&middot;&nbsp; [R] Reset &nbsp;&middot;&nbsp; [O] Obstacle &nbsp;&middot;&nbsp; [G] Target
      {/if}
    </div>
  </section>
  ...
  <aside class="sidebar-right">
    ...
    <div class="app-footer">
      ...
      <a href="https://github.com/Arziel1992/cos30002-tools/" target="_blank" rel="noopener noreferrer" style="color: var(--accent); text-decoration: none; font-weight: 600;">
...
<style>
  ...
  .badge-obstacle { color: #475569; border-color: #94a3b8; background: #e2e8f0; }
  .badge-target { color: #854d0e; border-color: #fde047; background: #fef08a; }

  hr { border: 0; border-top: 1px solid var(--panel-border); margin: 1.2rem 0; }
</style>
